...
namespace matcrm.service.Services
{
    public partial class EmployeeProjectService : Service<EmployeeProject>, IEmployeeProjectService
    {
        ...
        public List<EmployeeProject> GetAll(EmployeeProjectListRequest model)
        {
            if (!String.IsNullOrEmpty(model.SearchString))
            {
                var searchString = model.SearchString.ToLower();

                return _unitOfWork.EmployeeProjectRepository.GetMany(t => t.IsDeleted == false && ((t.Name.ToLower().Contains(searchString)) || (t.Description.ToLower().Contains(searchString)))).Result.Include(t => t.Status).Skip((model.PageNumber - 1) * model.PageSize).Take(model.PageSize).OrderBy(t => t.CreatedOn).ToList();


            }
            else
            {
                return _unitOfWork.EmployeeProjectRepository.GetMany(t => t.IsDeleted == false).Result.Include(t => t.Status).Skip((model.PageNumber - 1) * model.PageSize).Take(model.PageSize).ToList();
            }
        }

        public List<EmployeeProject> GetAllByTenant(long tenantId)
        {
            return _unitOfWork.EmployeeProjectRepository.GetMany(t => t.TenantId == tenantId && t.IsDeleted == false).Result.ToList();
        }

        public EmployeeProject GetEmployeeProjectById(long Id)
        {
            return _unitOfWork.EmployeeProjectRepository.GetMany(t => t.Id == Id && t.IsDeleted == false).Result.Include(t => t.Status).FirstOrDefault();
        }
    ...
    public partial interface IEmployeeProjectService : IService<EmployeeProject>
    {
        ...
        List<EmployeeProject> GetAll(EmployeeProjectListRequest model);
