...
namespace matcrm.api.Controllers
{
    ...
    public class EmployeeChildTaskController : Controller
    {
        ...
        public async Task<OperationResult<EmployeeChildTaskAddUpdateResponse>> Add([FromBody] AddUpdateEmployeeChildTaskRequest requestModel)
        {
            ...
            if (employeeChildTaskDto.AssignedUsers != null && employeeChildTaskDto.AssignedUsers.Count() > 0)
            {
                foreach (var userObj in employeeChildTaskDto.AssignedUsers)
                {
                    ...
                    if (!isExist)
                    {
                        var childTaskUserAssignDetails = _userService.GetUserById(employeeChildTaskUserObj.UserId.Value);
                        if (childTaskUserAssignDetails != null)
                            await sendEmail.SendEmailTaskUserAssignNotification(childTaskUserAssignDetails.Email, childTaskUserAssignDetails.FirstName + ' ' + childTaskUserAssignDetails.LastName, employeeChildTaskDto.Description, TenantId);
                        ...
                        employeeChildTaskActivityObj1.Activity = Enums.EmployeeTaskActivityEnum.Task_assigned_to_user.ToString().Replace("_", " ");
        ...
        {
            var assignChildTaskUserObj = _employeeChildTaskUserService.CheckInsertOrUpdate(childTaskUser);
        ...
        public async Task<OperationResult<AddUpdateEmployeeChildTaskTimeRecordResponse>> TimeRecord([FromBody] AddUpdateEmployeeChildTaskTimeRecordRequest requestModel)
        {
            ...
            if (employeeChildTaskTimeRecordDto.Duration != null && employeeChildTaskTimeRecordDto.EmployeeChildTaskId != null)
            {
                ...
                employeeChildTaskActivityObj.UserId = UserId;
                employeeChildTaskActivityObj.Activity = Enums.EmployeeTaskActivityEnum.Time_record_added.ToString().Replace("_", " ");
        ...
        public async Task<OperationResult<EmployeeChildTaskDto>> Remove(long ChildTaskId)
        {
            ...
            var documents = await _employeeChildTaskAttachmentService.DeleteAttachmentByChildTaskId(ChildTaskId);
        ...
        public async Task<OperationResult<EmployeeChildTaskVM>> Detail(long ChildTaskId)
        {
            ...
            var childTaskComments = _employeeChildTaskCommentService.GetAllByChildTaskId(employeeChildTaskVMObj.Id);
            if (childTaskComments != null && childTaskComments.Count > 0)
            {
                var childTaskCommentsVMList = _mapper.Map<List<EmployeeChildTaskCommentDto>>(childTaskComments);
                if (childTaskCommentsVMList != null && childTaskCommentsVMList.Count() > 0)
                {
                ...
                if (employeeChildTaskVMObj.Comments == null)
                {
                    employeeChildTaskVMObj.Comments = new List<EmployeeChildTaskCommentDto>();
                }
                employeeChildTaskVMObj.Comments = childTaskCommentsVMList;
            }
