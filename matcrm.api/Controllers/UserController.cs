...
namespace matcrm.api.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<OperationResult<UserSignUpResponseDto>> Register([FromBody] UserRegisterRequest model)
        {
            ...
            userSignUpResponseDto.Token = token.Value;
            userSignUpResponseDto.TenantId = user.TenantId;
            await sendEmail.SendNewSignUpEmail(requestmodel.Email, user.TempGuid, user.FirstName + ' ' + user.LastName, result.EmailOTP, user.TenantId, token.Value);
        ...
        public async Task<OperationResult<UserExternalLoginResponse>> ExternalLogin([FromBody] UserExternalLoginRequest externalAuth)
        {
            ...
            if (ischeck == false)
            {
                ...
                .AddClaims(GetClaim(userData, TenantName, RoleName))
                .Build();
        ...
        public async Task<FileResult> ProfileImageView(int Id)
        {
            ...
            if (userDetailsObj != null && !string.IsNullOrEmpty(userDetailsObj.ProfileImage))
            {
                ...
                if (System.IO.File.Exists(filePath))
                {
                    ...
                    return File(bytes, GetMimeTypes(userDetailsObj.ProfileImage), userDetailsObj.ProfileImage);
                }
                else
                {
        ...
        public async Task<OperationResult<UserInviteResponse>> Invite([FromBody] UserInviteRequest Model)
        {
            ...
            foreach (var emailObj in requestmodel.Emails)
            {
                ...
                if (isExistData == null)
                {
                    ...
                    userDto.CreatedBy = requestmodel.UserId;
                    userDto.TenantId = requestmodel.TenantId;
